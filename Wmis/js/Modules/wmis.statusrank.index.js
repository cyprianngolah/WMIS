...
wmis.statusrank.index = (function ($) {
	var statusRankTable;
	var options = {
	...
	function initDataTable() {
		var parameters;
		statusRankTable = $('#statusRank').dataTable({
			"iDisplayLength": 20,
			"scrollX": true,
			"bJQueryUI": true,
			"bProcessing": true,
			...
			"fnDrawCallback": function () {
				statusRankTable.$('tr.info').removeClass('info');
				$("#editButton").addClass('disabled');

				$("#statusRank tbody tr").click(function () {
					...
					if ($(this).hasClass('info')) {
						...
						statusRankTable.$('tr.info').removeClass('info');
						$(this).addClass('info');
						if (statusRankTable.$('tr.info').length) {
							// Get Data
							var position = statusRankTable.fnGetPosition(this);
							var data = statusRankTable.fnGetData(position);

							if (data.key) {
								$("#editButton").removeClass('disabled');
								$("#editButton").prop("href", "/StatusRank/Edit/" + data.key);
							}
						}
					}
				});
